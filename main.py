...
# coins = list(range(n)) # создаем список из монет
# random.shuffle(coins) # перемешиваем монеты
# flips = 0 # счетчик перевернутых монеток
# while any(coin == 0 for coin in coins): # пока не все монеты лежат одной стороной
#  coin = coins.pop(0) # переворачиваем первую монету
#  flips += 1 # увеличиваем счетчик перевернутых монет
